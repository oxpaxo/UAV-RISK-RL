...
def result_dir_from_args(args: argparse.Namespace | None = None) -> Path:
    if args is None:
        return DEFAULT_RESULT_DIR
    return ROOT / str(args.result_dir)


def write_stop(args: argparse.Namespace, reason: str, detail: str) -> None:
    result_dir = result_dir_from_args(args)
    ...
    if str(args.stop_flag_mode) == "confirm":
        flags = {
            ...
            "resource_saturation": "STOP_RESOURCE_UNSAFE.flag",
        }
    flag = flags.get(reason, flags["training_broken"])
    ...
    write_text(
        ...
        "\n".join(
            [
                "# Phase N3PF-STAB Training Partial Report",
                "",
...
def write_resource_affinity(stage: str, variant: str, seed: int) -> None:
    result_dir = result_dir_from_args(getattr(write_resource_affinity, "_args", None))
    table_prefix = getattr(write_resource_affinity, "_table_prefix", "phase_n3pf_stab")
    rows = [
    ...
    append_csv(result_dir / f"tables/{table_prefix}_resource_affinity.csv", rows)
...
def write_manifests(args: argparse.Namespace, cfg: dict[str, Any], out_dir: Path, model: PPO | None = None) -> None:
    ...
    append_csv(result_dir / f"tables/{table_prefix}_command_manifest.csv", rows)
    append_csv(result_dir / f"tables/{table_prefix}_config_manifest.csv", rows)
...
def train() -> None:
    ...
    result_dir = result_dir_from_args(args)
    table_prefix = str(args.table_prefix)
    write_resource_affinity._args = args  # type: ignore[attr-defined]
    write_resource_affinity._table_prefix = table_prefix  # type: ignore[attr-defined]
    ...
    try:
        ...
        append_csv(result_dir / f"tables/{table_prefix}_train_curve.csv", train_rows)
        append_csv(result_dir / f"tables/{table_prefix}_train_heartbeat.csv", heartbeat_rows)
        vec_env.close()
        ...
        write_stop(args, exc.reason, exc.detail)
        ...
        try:
            ...
            pass
        write_stop(args, "training_broken", detail)
        raise SystemExit(2)
