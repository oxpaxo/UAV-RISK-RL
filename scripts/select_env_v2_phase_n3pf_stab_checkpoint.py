...
def write_stop(result_dir: Path, args: argparse.Namespace, reason: str, detail: str) -> None:
    ...
    write_text(result_dir / str(args.report_file), f"# Phase N3PF-STAB Selector Partial Report\n\n`terminal_decision = {args.terminal_prefix}_stop_{reason}`\n\n```text\n{detail.strip()}\n```\n")
...
def main() -> None:
    ...
    try:
        by_seed_path = result_dir / f"tables/{table_prefix}_validation_eval_summary_by_seed.csv"
        ...
        for (variant, training_seed), group in scored.groupby(["variant", "training_seed"], dropna=False):
            improving, trend_detail = trend_for(group)
            choices = valid[(valid["variant"].astype(str) == str(variant)) & (pd.to_numeric(valid["training_seed"], errors="coerce").astype(int) == int(training_seed))].copy()
            if choices.empty:
                choices = group.copy()
        ...
        write_csv(result_dir / f"tables/{table_prefix}_validation_checkpoint_scores.csv", scored.to_dict("records"))
        write_csv(result_dir / f"tables/{table_prefix}_selector_decision.csv", selected_rows)
        ...
        write_stop(result_dir, args, "hard_error", detail)
        raise SystemExit(2)
