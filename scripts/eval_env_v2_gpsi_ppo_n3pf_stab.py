...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run", action="append", required=True, help="variant:training_seed:config_path:checkpoint_dir")
...
def parse_run(text: str) -> dict[str, Any]:
    parts = text.split(":", 3)
    if len(parts) != 4:
        raise StabEvalStop("hard_error", f"--run must be variant:training_seed:config:checkpoint_dir, got {text!r}")
    variant, seed_text, config_path, checkpoint_dir = parts
    return {
        "variant": variant,
...
def write_checkpoint_manifest(result_dir: Path, rows: list[dict[str, Any]]) -> None:
    append_csv(result_dir / "tables/phase_n3pf_stab_checkpoint_manifest.csv", rows)
...
def main() -> None:
    ...
    try:
        ...
        write_checkpoint_manifest(result_dir, manifest_rows)
        if not episode_rows:
        ...
        write_csv(result_dir / f"tables/phase_n3pf_stab_{args.eval_phase}_episode_metrics.csv", episode_rows)
        write_csv(result_dir / f"tables/phase_n3pf_stab_{args.eval_phase}_eval_summary_by_seed.csv", base_eval.summarize_episode_rows(episode_rows, by_seed_cols))
        write_csv(result_dir / f"tables/phase_n3pf_stab_{args.eval_phase}_scenario_breakdown.csv", base_eval.summarize_episode_rows(episode_rows, scenario_cols))
        write_csv(result_dir / f"tables/phase_n3pf_stab_{args.eval_phase}_motion_mode_breakdown.csv", base_eval.summarize_episode_rows(episode_rows, by_seed_cols + ["threat_motion_mode"]))
        write_csv(result_dir / f"tables/phase_n3pf_stab_{args.eval_phase}_threat_class_breakdown.csv", base_eval.summarize_episode_rows(episode_rows, by_seed_cols + ["threat_class"]))
        write_csv(result_dir / f"tables/phase_n3pf_stab_{args.eval_phase}_raw_unsafe_action_steps.csv", raw_rows_all)
        write_csv(result_dir / f"tables/phase_n3pf_stab_{args.eval_phase}_raw_unsafe_action_summary.csv", summarize_raw(raw_rows_all))
        write_csv(result_dir / f"tables/phase_n3pf_stab_{args.eval_phase}_gpsi_output_summary.csv", gpsi_summary_rows(gpsi_accum))
        write_csv(result_dir / f"tables/phase_n3pf_stab_{args.eval_phase}_feature_block_stats.csv", feature_summary_rows(feature_accum, cfg_by_method))
        append_csv(
            result_dir / "tables/phase_n3pf_stab_eval_command_manifest.csv",
            [
                {
                    "eval_phase": args.eval_phase,
