...
STOP_FLAGS = {
    ...
    "checkpoint_missing": "PHASE_N3R_STOP_TRAIN_FAILED.flag",
...
DEFAULT_CONFIGS = {
    "raw_z": {
        ...
        "checkpoint_dir": "checkpoints/env_v2_gpsi_heada_ppo_n3r_raw_z_s0",
    },
    "z_norm": {
        "config": "configs/env_v2_gpsi_heada_ppo_n3r_z_norm.yaml",
        "checkpoint_dir": "checkpoints/env_v2_gpsi_heada_ppo_n3r_z_norm_s0",
    },
    "no_z": {
        "config": "configs/env_v2_gpsi_heada_ppo_n3r_no_z.yaml",
        "checkpoint_dir": "checkpoints/env_v2_gpsi_heada_ppo_n3r_no_z_s0",
    },
}


class PhaseN3REvalStop(Exception):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate Phase N3R repaired Gpsi-PPO configs.")
    parser.add_argument("--result-dir", default="results/env_v2_phase_n3r_gpsi_ppo_rerun")
    parser.add_argument("--configs", nargs="+", default=["raw_z", "z_norm", "no_z"])
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "",
                "Partial report generated by the N3R eval script.",
                "",
                "```text",
                detail.strip(),
                "```",
...
def load_yaml(path: Path) -> dict[str, Any]:
    if not path.exists():
        raise PhaseN3REvalStop("schema_mismatch", f"missing config: {rel(path)}")
    with path.open("r", encoding="utf-8") as handle:
        payload = yaml.safe_load(handle)
    if not isinstance(payload, dict):
        raise PhaseN3REvalStop("schema_mismatch", f"config is not a mapping: {rel(path)}")
    return payload
...
def make_gpsi_env(cfg: dict[str, Any], scenario: str, args: argparse.Namespace) -> GpsiObsWrapper:
    ...
    if not checkpoint.exists():
        raise PhaseN3REvalStop("gpsi_checkpoint_missing", f"missing Gpsi checkpoint: {rel(checkpoint)}")
    ...
    if freeze.training or freeze.requires_grad_any or freeze.trainable_parameters != 0:
        raise PhaseN3REvalStop("gpsi_not_frozen", f"Gpsi freeze check failed in eval: {freeze}")
    expected = int(gpsi_cfg.get("obs_aug_dim", 94))
    actual = int(wrapped.observation_space["obs"].shape[-1])
    if actual != expected:
        raise PhaseN3REvalStop("schema_mismatch", f"expected aug obs dim {expected}, got {actual}")
    return wrapped
...
def add_gpsi_diagnostics(
    *,
    ...
    if active.size:
        ...
        if include_z:
            blocks["z_i_64_after_norm"].add(z_after[active])
...
def feature_summary_rows(
    ...
    for key, groups in feature_accum.items():
        ...
        include_z = bool(cfg_by_method.get(str(method_key), {}).get("gpsi", {}).get("include_z", True))
        for block in ["obs_i_12", "z_i_64_raw", "z_i_64_after_norm", "delta_hat_9_after_scale", "logvar_hat_9_clamped", "full_aug_obs"]:
            if block == "z_i_64_after_norm" and not include_z:
...
def checkpoint_specs(config_key: str, cfg: dict[str, Any]) -> list[tuple[str, Path, int, str]]:
    ...
    ckpts = [
        (f"{config_key}_250k", ckpt_dir / "checkpoint_250k.zip", 250_000, "250k"),
        (f"{config_key}_final", ckpt_dir / "final.zip", train_steps, "final"),
    ]
    if train_steps != 250_000 and not ckpts[1][1].exists():
        ckpts[1] = (f"{config_key}_{train_steps // 1000}k", ckpt_dir / f"checkpoint_{train_steps // 1000}k.zip", train_steps, "final")
    return ckpts
...
def main() -> None:
    ...
    try:
        ...
        if not attention_path.exists():
            raise PhaseN3REvalStop("checkpoint_missing", f"missing attention reference checkpoint: {rel(attention_path)}")
        append_csv(
            result_dir / "tables/phase_n3r_command_manifest.csv",
            [
                {
                    "stage": "eval",
                    "method_key": "all",
        ...
        for key in args.configs:
            if key not in DEFAULT_CONFIGS:
                raise PhaseN3REvalStop("schema_mismatch", f"unknown config key: {key}")
        ...
        write_csv(result_dir / "tables/phase_n3r_config_manifest.csv", config_rows)
        ...
        for key in args.configs:
            ...
            for checkpoint_name, checkpoint_path, fallback_step, label in checkpoint_specs(key, cfg):
                if not checkpoint_path.exists():
                    raise PhaseN3REvalStop("checkpoint_missing", f"missing checkpoint for {key}: {rel(checkpoint_path)}")
        ...
        for method_key, method, cfg, checkpoint_name, checkpoint_path, step_value, checkpoint_label in method_jobs:
            ...
            for scenario in args.scenarios:
                ...
                last = start
                print(
                    f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] N3R_EVAL_START "
                ...
                for episode_id in range(args.num_episodes):
                    ...
                    if now - last >= args.heartbeat_seconds or episode_id == args.num_episodes - 1:
                        ...
                        print(
                            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] N3R_EVAL_HEARTBEAT "
        ...
        write_csv(result_dir / "tables/phase_n3r_episode_metrics.csv", all_episode_rows)
        write_csv(result_dir / "tables/phase_n3r_checkpoint_eval_summary.csv", eval_summary)
        write_csv(result_dir / "tables/phase_n3r_eval_summary.csv", eval_summary)
        write_csv(result_dir / "tables/phase_n3r_scenario_breakdown.csv", eval_summary)
        write_csv(
            result_dir / "tables/phase_n3r_motion_mode_breakdown.csv",
            summarize_episode_rows(all_episode_rows, ["method_key", "method", "checkpoint", "checkpoint_step", "checkpoint_label", "threat_motion_mode"]),
        )
        write_csv(
            result_dir / "tables/phase_n3r_threat_class_breakdown.csv",
            summarize_episode_rows(all_episode_rows, ["method_key", "method", "checkpoint", "checkpoint_step", "checkpoint_label", "threat_class"]),
        )
        write_csv(result_dir / "tables/phase_n3r_raw_unsafe_action_steps.csv", all_raw_rows)
        write_csv(result_dir / "tables/phase_n3r_raw_unsafe_action_summary.csv", summarize_raw(all_raw_rows))
        write_csv(result_dir / "tables/phase_n3r_gpsi_output_summary.csv", gpsi_summary_rows(gpsi_accum))
        write_csv(result_dir / "tables/phase_n3r_aug_feature_block_stats.csv", feature_summary_rows(feature_accum, cfg_by_method))
        copy_sample_traces(result_dir, all_trace_files)
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] N3R_EVAL_END "
            f"episodes={len(all_episode_rows)} raw_steps={len(all_raw_rows)} gpsi_groups={len(gpsi_accum)} traces={len(all_trace_files)}",
            flush=True,
        )
    except PhaseN3REvalStop as exc:
